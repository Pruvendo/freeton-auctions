...
def contract(pytestconfig):
    ...
    auction_data = ts4.load_data_cell('Auction.tvc')
    return ts4.BaseContract(
        'AuctionRoot',
        dict(code=auction_code, data=auction_data),
...
def test_deploy_auction(contract):
    ...
    contract2 = ts4.BaseContract('Auction', ctor_params=None, address=auction_address)
